...
async function seed() {
  ...
  try {
    ...
    // Keep at least one deterministic account for curl/Postman examples.
    const studentEmails = [
   ...
   VALUES ($1, false, 5, 0, CURRENT_DATE)
    ...
    // Summary
    const {
      rows: [{ count: usersCount }],
    } = await client.query("SELECT COUNT(*)::int AS count FROM users");
    const {
      rows: [{ count: coursesCount }],
    } = await client.query("SELECT COUNT(*)::int AS count FROM courses");
    const {
      rows: [{ count: lessonsCount }],
    } = await client.query("SELECT COUNT(*)::int AS count FROM lessons");
    const {
      rows: [{ count: questionsCount }],
    } = await client.query("SELECT COUNT(*)::int AS count FROM questions");
    ...
    const {
      ...
      ? await client.query(
          `SELECT id, title FROM quizzes WHERE lesson_id = $1 ORDER BY created_at ASC LIMIT 1`,
          [firstLessonId],
        )
      : { rows: [null] };
    ...
    console.log(
      "STUDENTS:",
      students.map((s) => ({ id: s.id, email: s.email, role: "student" })),
    );
    ...
    if (firstLessonId) {
      console.log("GET  /api/quiz/:lessonId/start", {
